...
- `nsf.diagnostics.mode`: 诊断强度，推荐使用 `basic`、`balanced` 或 `full`
...
- 预处理宏 preset 会首次写入 `nsf.preprocessorMacros` 工作区设置；设置里看到的宏表就是实际 preset，`nsf.defines` 和源码 `#define/#undef` 会继续覆盖它。
