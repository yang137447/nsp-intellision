...
- diagnostics 分成两层：先发布低成本的 immediate syntax，再补齐较重的 deferred semantic。
...
- inlay hints 属于 deferred 路径；当 full-document inlay snapshot 尚未就绪时，请求当前可见范围会优先走 visible-range first 构建，full-document cache 仍在后台补齐。
