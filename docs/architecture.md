...
- `server_parse.*`: 共享行级 declaration/header 解析、`for` initializer declaration 解析、宏定义头解析、注释/字符串剥离后的 shared line scan、多行 nesting 前后状态，以及 missing-semicolon syntax boundary 判断；declaration parser 会把 declarator array suffix 作为 `T[]` 保留在 consumer-ready 类型上，供 expression typing 区分数组 indexing 与 matrix row indexing；多行函数 / control header、NSF metadata / effect block、表达式 continuation 和 macro-only recovery 区域的高置信缺分号判定应在这里统一收敛
...
- `workspace/workspace_index_extract.*`: struct / definition 提取，包括 FX/NSF metadata-block 全局变量声明；struct member 提取消费共享 declaration parser，必须保留 `T[]` declarator array suffix 进入 workspace summary
