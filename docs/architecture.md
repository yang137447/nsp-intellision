...
| consumer-ready 语义接口 | `scalar_type_model.*`、`type_model.*`、`semantic_snapshot.*` 等查询结果 | 提供下游可直接消费的共享判定 |
...
- `diagnostics/*`: diagnostics facade、semantic rules、expression type、symbol type、emit、preprocessor、syntax 和 indeterminate 分层；semantic diagnostics 的 local symbol、duplicate local declaration、`for` initializer、statement-like macro 声明局部变量可见性和基础 block-flow 诊断基于函数内 lexical scope stack，duplicate local 只在同一 lexical scope 且 active preprocessor branch 重叠时发布；block-flow diagnostics 只把顺序执行路径上的 `return` 传播为后续 unreachable，`if/else`、loop 和 switch 的 branch-local return 不得污染 sibling branch 或后续可达语句；unbraced `if/else` 的 pending controlled statement 必须跨 blank / comment-only trivia 保留，并按同一 statement 段和 pending `else` 标记判断 return 是否属于 branch-local 或完整 if/else 双臂返回；semantic unreachable 发布还必须消费 `PreprocessorView` active line / branch merge 信息，避免 inactive `#if/#else` 分支产生用户可见语义不可达误报；object method call diagnostics 必须消费 `hover_markdown.*` 展开的资源参数和 `type_relation.*`，不得在 rule 内本地推导 array texture 坐标或复制 sampler / coord 兼容规则
...
- `diagnostics_expression_type.*`: diagnostics 共享表达式类型 helper；负责类型 token 归一化、builtin call 类型规则、active object-like macro replacement 的表达式类型入口、numeric literal token-span 解析和表达式结果类型推断。declaration-side 类型 token 和 expression-side macro-like constructor 都应优先消费当前行 active macro replacement，例如 `MaterialFloat4` 在不同 active branch 下按实际 replacement 归一为 `half4` 或 `float4`；无 active replacement 时才消费 `type_desc.*` 的共享轻量 alias 形状，scalar/vector/matrix 形状由 `scalar_type_model.*` 解释，不在 diagnostics rule 层写项目名特判。builtin call 的元素类型合并和 mixed signedness 判断应通过 `type_relation.*` 的 usual arithmetic conversion 产生合法转换 warning，不能把 `int` / `uint` 混用伪装成 builtin mismatch。common builtin 规则包括同型返回的 `log/log2/log10/round/trunc/cosh/radians/degrees/ddx/ddy/fwidth`，返回 bool 的 `all/any/isnan`，矩阵转置 `transpose`，call-only 的 `clip` / `sincos`，以及返回第一浮点参数同形类型且要求第二个 out 参数同形同元素类型的 `modf`；`uint64_t` shift / bitwise-or 等表达式必须在该共享入口保留 64-bit 整型结果；数组后缀 `T[]` 必须先于 vector/matrix indexing 被消费，避免 `float4x4[]` 一次索引后被误降为 matrix row；`mul(matrix, vector)` 允许项目 shadercompiler 已确认的矩阵列数向量维度 lowering（例如 `float3x3 * half2` 返回 `float3`），但不允许反向 `vector * matrix` 或向量维度超过矩阵列数；参数类型无法推断时仍发布 indeterminate metadata，而不是回退到猜测类型。numeric literal 解析必须在该共享入口按官方 HLSL numeric literal 语法处理，因为当前 lexer 会把 decimal point 和 exponent sign 切为 punctuation token，semantic diagnostics 不应在规则层复制后缀判断；合法 exponent、leading/trailing dot、octal / hex integer、`h/H/f/F/l/L` 浮点 suffix 和 `u/U/l/L` 整数 suffix 组合都应在这里统一判定。implementation-only `ll/ull` 整数 suffix 作为历史写法只应由共享入口产生 warning 并推荐 `l/ul`，真正不符合语法的 suffix 继续作为 error。
- `type_desc.*`: diagnostics / overload 共享轻量类型形状解析；负责把 HLSL scalar / vector / matrix / object token、项目 64-bit integer scalar alias（`int64_t` / `uint64_t`）和常见 macro-like numeric alias（如 `MaterialFloat3`、`MaterialHalf4x4`）归一为 `TypeDesc`。它通过 `scalar_type_model.*` 消费 scalar/vector/matrix 资源，不负责完整 typedef 展开、用户 struct 建模或对象方法坐标维度，这些仍由 semantic snapshot、symbol query 和 `type_model.*` 负责。
- `type_relation.*`: diagnostics / overload 共享 HLSL 隐式转换模型；以官方 standard conversion sequence、usual arithmetic conversions 和 overload conversion rank 为基础，结构化返回 compatible / incompatible、conversion kind、cost 和 risky implicit conversion warning。assignment、return、user function argument、builtin argument、object method argument 和 binary operator diagnostics 不应再各自复制 half/float、scalar splat、component-wise conversion、truncation、signedness 或 boolean conversion 判断；`int64_t` / `uint64_t` 这类项目 64-bit integer scalar alias 也应通过该共享模型参与整型合并与 signed / unsigned 组合判断。合法但有风险的隐式转换发布独立 warning；找不到官方转换序列时才发布 type mismatch error。
...
- `scalar_type_model.*`: `types/scalar_types` 的统一加载、缓存和 scalar/vector/matrix type-name / shape 查询
...
- `workspace/workspace_index_cache.*`: cache 路径、磁盘 load/save 和旧索引兼容迁移；workspace index key 包含资源 model hash 与声明提取语义版本，parser / extraction contract 改变时必须 bump 提取语义版本，避免旧 workspace symbol cache 继续暴露已修复的错误定义
...
- `workspace/workspace_index_extract.*`: struct / definition 提取，包括 FX/NSF metadata-block 全局变量声明；top-level variable、cbuffer member 和 struct member 提取消费共享 declaration parser，必须保留 `T[]` declarator array suffix 进入 workspace summary，并且 object template declaration 只索引真实 declarator name，不把 `<...>` 内的 type argument 记录成 workspace symbol；函数定义仍由函数提取路径记录，不由变量 declaration 结果重复索引
...
- `server_cpp/src/macro_statement_locals.hpp`: statement-like macro local 提取边界；只暴露 active object-like macro 独占语句行中的声明型 replacement，不执行通用文本展开
- `server_cpp/src/scalar_type_model.hpp`: scalar/vector/matrix type-name 与 shape 查询边界；对象类型语义仍属于 `type_model.*`
...
- HLSL scalar / vector / matrix 类型名与形状：`scalar_type_model.*` + `server_cpp/resources/types/scalar_types/`
...
- `types/scalar_types`: scalar/vector/matrix completion 候选、semantic token type 分类、callsite constructor 识别、`type_desc.*` 与 diagnostics 表达式类型形状解析
- `types/object_types` / `types/object_families` / `types/type_overrides`: texture / sampler / buffer 家族识别、成员方法匹配和 diagnostics 类型兼容辅助
