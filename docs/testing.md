...
- `analysis-context-unit-profile` 当前同时覆盖 `gimlocalvariants.json` 命中、`used_shader_variants.csv` fallback 命中、`active_unit_variant_selection.csv` 按 unit stem 的 row 选择提示命中、`nsf.shaderCompilerPath` 额外发现根命中，以及“冲突 profile 宏不猜默认；仅作为 unresolved profile metadata 暴露”的边界；并覆盖 workspace 显式宏（`nsf.defines` / `nsf.preprocessorMacros`，含符号链解析为数值）作为 selection hint 覆盖/收敛 profile row 的路径，断言 profile 总行数 / 筛选后行数 / 单行命中 signature / selection hint source path 元数据
...
- `npm run test:client:real:replay`
  ...
  - 顶层 `captureCompletion` / `captureSignatureHelp` / `captureDefinition` step 和完整文件输入 probe 都会进入 `latencySummary`。definition replay 会记录 `latencySummary.definition.capture`、`locationCount` 和最慢跳转；`captureDefinition` / `captureReferences` 可用 `expectedLocationMatches` 校验目标 URI、字段/函数名 range 文本和目标行文本，并可用 `maxDurationMs` 设置单次 wall-clock 预算。expected 缺失、数量越界、目标文本不匹配或耗时超预算都会进入 replay anomaly 并使测试失败。signature replay 的 `latencySummary.signatureHelp` 包含 `uiLatestVisibleProviderReturn`、`uiLatestVisibleLspRequest`、`postLatestVisibleProviderActivity`、`postLatestVisibleQuietGuard` 和 `uiQueueQuietGuard`；分析 signature help 尾延迟时应优先区分 latest visible provider/LSP 是否慢，以及 latest visible 返回后的时间是仍有 provider activity 还是只是 replay queue-quiet guard。
...
- real workspace diagnostics audit
  ...
  - 报告 summary 会记录 `macroHealth`，字段口径来自 server 共享 `PreprocessorView`：`initialConfiguredMacroCount`、`initialArtDefaultZeroMacroCount`、`initialCompilerPrivateConstantCount`、`initialCompilerMacroSnapshotCount`、`initialNumericDefineCount`、`initialMacroCount`、`sourceDefineEvents`、`ifndefDefaultDefineEvents`、`sourceUndefEvents`、`synthesizedZeroEvents`、`conditionDiagnosticCount`、`undefinedMacroDiagnosticCount`、`expansionWarningDiagnosticCount`、`inactiveBranchDiagnosticCount`、`branchMergeCount` 和 `activeIncludeCount`。这些字段是 audit / debug 健康度指标，不改变 diagnostics 发布策略；`initialArtDefaultZeroMacroCount` 表示 workspace summary 提供的 `#art` BOOL/INT default-zero 初始宏输入数量，且可能包含当前 active include closure 内无冲突的 `#art` companion enum 常量，`initialCompilerPrivateConstantCount` 表示当前 active closure 内作为初始输入的 compiler private numeric constants 数量，`initialCompilerMacroSnapshotCount` 表示当前 active closure 内由 C++ compiler macro snapshot 注入的稳定 alias / default alias 数量。source define / undef 事件按 unit/include context 中可见的宏状态事件计数，事件会保留真实 source location 供 hover / definition 使用。real diagnostics audit 会用显式 cache scope 在同一 active unit 的多文件扫描中复用 scoped `#art` companion 输入、compiler private constants 和 compiler macro snapshot，并在 cache 命中时跳过重复的 active-unit strict 预扫描；普通 LSP publish/interactive 路径不启用该 audit cache scope。报告还会输出 `macroFocus`，当前固定跟踪 `COLOR_CHANGE_MODE`、`EMISSIVE_MODE` 和 `FOLIAGE_MODE` 的剩余 undefined diagnostics、affected units、affected files、owner/source boundary、样例行以及 per-unit active-unit compile profile / `#art` default-zero evidence/status summary，用来区分 focus macro 是已注入、`art-default-zero`、profile unresolved、profile source missing / empty / no selected row / no macro，还是 runtime debug 缺失。报告还会输出 `compilerContextMacroEvidence`，当前固定跟踪 P14L compiler context / platform-quality 宏，逐项记录 server 默认 preset、当前 effective `nsf.preprocessorMacros`、`nsf.defines` 和 per-unit active-unit profile 是否提供该宏；该 evidence 只用于解释旧 preset 漂移、profile 缺失或默认 preset 缺口，不会自动补值或改变 diagnostics 行为。
...
- 改共享 preprocessor 状态机（例如 `#define/#undef/#ifdef/#ifndef/defined(...)`、include 顺序、空 replacement、符号链 / 用户配置表达式求值、function-like macro 展开、变参 / `__VA_ARGS__`、`##` / `#`、缺失叶子宏补 `0`、`#art` default-zero / closure-scoped companion enum 初始宏输入、compiler private numeric constant 初始宏输入、compiler macro snapshot 初始宏输入、`#undef` 清空候选后由后续稳定 `#define` 重建初始宏候选、macro hover / definition 共享查询或 macroHealth audit 指标）：至少补跑 `cmake --build .\server_cpp\build`、diagnostics repo 集成用例、相关 interactive hover / definition repo 用例、5-unit smoke audit 和 50-unit trend audit；如果只是 focused 语义子阶段且未落地 audit 指标，阶段记录必须说明 audit 指标仍属后续子阶段
...
`pbr-flow-water-full-input` 会从空 buffer 输入完整节点文件，按成员补全、texture method、函数族补全、内置函数、局部变量、uniform、预处理宏、signature help 和最终 diagnostics 分组采样；完整输入完成并等待 client quiescent 后，还会覆盖本文件函数、include 函数、workspace helper、结构体字段、inline include 字段、预处理宏、include path，以及 `uber_fx_common.nsf` 中 `batch.emissivecolor` / `MaterialInputs.base_color` 等真实跳转回归点。该脚本属于重型验证，单次运行可能需要数分钟。
